...
def retrieve_logit_lens_internvl(state, img_path, text_prompt=None, num_patches=1):
    ...
    hidden_states = torch.stack(output.hidden_states[0][-3:]).unsqueeze(0)
    ...
    with torch.inference_mode():
        curr_layer_logits = state["model"].get_output_embeddings()(hidden_states).to(torch.bfloat16).cpu().float()
